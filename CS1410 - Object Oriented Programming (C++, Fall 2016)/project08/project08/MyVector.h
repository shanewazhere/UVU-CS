...
// Class: CS1410 001
// Date Written: 11/10/2016
// Description: Class Template file
// -----------
//I declare that the following source code was written solely by me.
//I understand that copying any source code, in whole or in part, 
// constitutes cheating, and that I will receive a zero on this project
...
template <class T>
MyVector<T>::MyVector(const T& size)
{
	vectorSize = 0;
	vectorCapacity = size;
	//double the size to get the capacity of the array. half empty cells;
...
MyVector<T>::MyVector(const MyVector<T>& b)
{
	...
	{
		this->theArray[i] = b.theArray[i];
	}
}

// Overloaded assignment operator
template <class T>
MyVector<T>& MyVector<T>::operator=(const MyVector<T>& assigning)
{
...
{
	const T DEFAULT_SIZE = 2;
	T* resetArray = NULL;
	vectorSize = 0;
	vectorCapacity = DEFAULT_SIZE;
	resetArray = new T[vectorCapacity];
...
void MyVector<T>::push_back(const T& n)
{
	if (size() != capacity())
	{
		...
		if (size() == capacity())
		{
			...
			T* newArray = NULL;
			newArray = new T[capacityNewArray];
...
template <class T>
ostream& operator<<(ostream& out, MyVector<T> &vectorObj)
{
